...
tests whether the project-card variables of wind, wave height, and relative
...
The rotor model is best understood as a controlled comparison rather than a
direct measurement. The historical AIS data represent actual vessel behaviour,
but they do not provide a clean experimental switch that tells us exactly when
the rotor was off and on under identical conditions. The project therefore
builds a baseline prediction and then applies a consistent rotor contribution
derived from the supplied polar relationship. This creates a repeatable
baseline-versus-rotor comparison for the same rows.
...
questions in the project card. Because the assumption is visible, it can later
...
contribution. It also lets the same baseline prediction be compared with and
without rotor assistance. That makes the difference easier to explain in the
...
rotor telemetry. This is acceptable for a what-if prototype, but future work
should validate the scenario against measured rotor power, shaft power, fuel
flow, or controlled before/after periods if such data become available.
...
The AI did not have access to the raw data and did not make any modelling decisions. 
