...
50.72 t on the observed route to 47.81 t under sampled metocean scenarios.
...
Monte Carlo routing & Expected-cost route for Voyage 364: 47.81 t mean rotor-assisted fuel and 85.6\% probability of beating the shortest feasible route. \\
\bottomrule
\end{tabularx}
\end{table}
...
\section{Monte Carlo routing extension}

The latest extension turns route optimisation into an uncertainty-aware routing
problem. Instead of choosing a route under one averaged weather condition, the
notebook samples many plausible metocean scenarios for each graph edge from the
historical feature table. The sample is empirical: records are grouped into
0.7 degree spatial cells, the voyage month plus or minus one month is preferred
to preserve seasonality, and fallback pools are used when the local cell has too
few observations. Each sample contains true wind speed, wind direction,
significant wave height \hs{}, and eastward and northward current components
\texttt{uo} and \texttt{vo}.

The reference case is Voyage 364. The observed itinerary is split into four
sailing legs using stop gaps longer than two hours, so the optimisation keeps
the voyage structure instead of replacing it with one unrealistic direct line.
The route area is converted into a navigable sea graph with 0.35 degree grid
spacing. Nodes are kept near historically observed AIS water corridors, edges
connect neighbouring cells in eight directions, and feasibility filters remove
nodes or edges that are too far from observed water traffic or cross land under
the Natural Earth land mask. The current graph has 537 nodes, 3,784 edges, and
132 AIS-corridor override edges for near-port movements.

Each edge is evaluated under every sampled scenario. Current is projected onto
the route course, wave height applies a speed penalty above a calm-water
threshold, rotor power is estimated from the same polar-based scenario used in
the rest of the project, and the resulting assisted speed is converted into
fuel and \co{} with the simplified energy model. This gives each edge a
distribution of possible costs rather than one deterministic cost.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{monte_carlo_routing.png}
...
\small
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.27\textwidth}rrrr}
\toprule
...
The main operational result is that the shortest feasible route is not the best
route under the sampled fuel objective. The expected-cost Monte Carlo route
accepts a small distance increase in exchange for more favourable assisted
propulsion conditions and beats the shortest feasible route in 85.6\% of
samples. The risk-aware route is the same path in the current run, meaning the
chosen risk penalty does not change the optimum. The scenario-optimal bound is
not a deployable recommendation; it is an optimistic benchmark where each
sample is allowed to choose its best route after the weather is known.
...
high-gain operational rules. The Monte Carlo extension adds uncertainty-aware
route comparison, so the routing result can be discussed as a distribution of
possible outcomes rather than a single deterministic path.
...
At route level, the Monte Carlo result reinforces the same point. The best route
is not necessarily the shortest route; it is the route that performs well across
sampled wind, wave, and current conditions while respecting the observed
itinerary and navigable corridor constraints.
